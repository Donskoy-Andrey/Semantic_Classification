...
const ZipUploadComponent = ({docsNumber, openModal}) => {
    ...
    const handleFileChange = async (files) => {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_BACKEND}/upload`, formData, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            });
            setUploadedFile(response.data); // Assuming response.data contains the uploaded file information
        } catch (error) {
        ...
        <div className="main-page">
            <div className="container mt-4 main-bg">
                ...
                <div className="input-control__buttons">
                    ...
                    <button className="btn btn-success modal-button" onClick={openModal}>
                        Примеры
                    </button>
                </div>
            </div>
